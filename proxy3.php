...
$local = @fsockopen("localhost", 9666, $errno, $errstr, 3);
...
do {
    ...
    if($chunk === false) {
        ...
        if (parseLastHeader($rest)) {
            ...
            $rest = preg_replace('/src="\//', 'src="proxy3.php?req=/', $rest);
            $rest = preg_replace('/href="\//', 'href="proxy3.php?req=/', $rest);
            fwrite($extOut, $rest);
            $rest = '';
            debug_log("passed last headers");
        }
    } else {
        $chunk = preg_replace('/src="\//', 'src="', $chunk);
        debug_log("FFFFFFFFFFFFFFFFFFFFFF : " .$chunk);
        fwrite($extOut, $chunk);
